...
export default function SharedTrip() {
  ...
  if (!data) return <View style={{ flex: 1 }}><AmbientBackground /><SafeAreaView style={{ flex: 1 }}><Empty icon="link-outline" title="Tautan tidak valid" subtitle="Tautan pantau perjalanan salah atau sudah tidak tersedia." action={<Button title="Buka Antar Aja" onPress={() => router.replace('/')} />} /></SafeAreaView></View>;
  ...
  return (
    ...
    >
      <View style={{ gap: 12 }}>
        {d ? (
          <View style={s.card}>
            <Row gap={12}>
              ...
              <View style={s.plate}><Text style={{ fontWeight: '900', color: '#fff', letterSpacing: 1 }}>{d.plate}</Text></View>
            </Row>
            <Row gap={8} style={{ marginTop: 8 }}><Ionicons name="shield-checkmark" size={16} color={colors.success} /><Text style={font.tiny}>Mitra terverifikasi Antar Aja · posisi diperbarui otomatis</Text></Row>
          </View>
        ...
        {Platform.OS === 'web' && <Button title="Unduh / buka Antar Aja" variant="secondary" onPress={() => router.replace('/')} />}
      </View>
    </MapScreen>
  );
}
const s = StyleSheet.create({
  ...
  card: { backgroundColor: 'rgba(255,255,255,0.7)', borderRadius: radius.lg, padding: 14, borderWidth: 1, borderColor: glass.border },
